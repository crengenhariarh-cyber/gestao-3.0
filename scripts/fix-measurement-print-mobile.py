...
# Remove any earlier popup-based print function if present.
...
print_fn = r'''  async function finalizeMeasurement(){if(!activeMeasurementId){setError('Salve ao menos um serviço antes de finalizar a medição.');return;}try{await saveHeader();await operations.setMeasurementStatus(activeMeasurementId,'close');onChanged();onClose();}catch{return;}}
  function printMeasurement(){
    ...
    const existing=document.getElementById('measurement-print-root');
    existing?.remove();
    ...
    if(logo&&!logo.complete){let fired=false;const done=()=>{if(fired)return;fired=true;trigger();};logo.addEventListener('load',done,{once:true});logo.addEventListener('error',done,{once:true});setTimeout(done,1200);}else{trigger();}
    setTimeout(()=>{if(document.getElementById('measurement-print-root'))cleanup();},10000);
  }
  function closeFlow(){onClose();}'''
...
print('Mobile-safe measurement print patch applied.')
