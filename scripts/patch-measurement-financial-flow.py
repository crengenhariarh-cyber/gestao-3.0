...
current="case 'measurement':content=shell(<>{!focusedContractId&&select('Contrato','contractId',contractOptions,true)}{input('Competência','competence','month',true)}{input('Nº medição','measurementNumber','text',true)}{input('Vencimento previsto','dueDate','date')}{input('Data prevista para pagamento','expectedPaymentDate','date')}{input('Forma de pagamento','paymentMethod')}{input('Torre / Aditivo','originLabel')}{input('Observações','notes')}</>,'INSS, ISS e retenção técnica serão carregados automaticamente conforme o contrato. Os dados financeiros acompanharão a medição até o contas a receber.');break;"
if current in s:
    s=s.replace(current,new)
else:
    s=replace_required(s,old,new,'measurement form')
s=replace_required(s,'<Dialog open={kind!==null} title={kind?titles[kind]:\'Engenharia\'}','<Dialog open={kind!==null} variant={kind===\'measurement\'?\'measurement-fullscreen\':undefined} title={kind?titles[kind]:\'Engenharia\'}','measurement fullscreen dialog')
s=replace_required(s,'confirmLabel="Salvar">','confirmLabel={kind===\'measurement\'?\'Salvar e continuar\':\'Salvar\'}>','measurement continue label')
p.write_text(s)
