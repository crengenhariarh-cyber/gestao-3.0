...
new_header = "    content=<div className=\"engineering-contract-workspace__page engineering-sheet\"><header className=\"engineering-sheet__head\"><div><small>{meta.eyebrow}</small><h3>{meta.title}</h3><p>{meta.description}</p></div><div className=\"engineering-sheet__head-actions\"><span className=\"engineering-sheet__contract-badge\">{contract.contractNumber}</span>{section==='contrato'&&<Button size=\"sm\" onClick={()=>setContractRetentionEditOpen(true)}>Editar contrato</Button>}</div></header>{body}</div>;"
if old_header in s:
    s = s.replace(old_header, new_header, 1)
elif 'engineering-sheet__head-actions' not in s:
...
css = Path('src/styles/engineering.css')
css_text = css.read_text()
addition = '\n.engineering-sheet__head-actions{display:flex;align-items:center;gap:12px;flex-wrap:wrap;justify-content:flex-end}\n.engineering-sheet--modal{min-height:0}\n'
if '.engineering-sheet__head-actions{' not in css_text:
