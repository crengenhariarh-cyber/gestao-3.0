...
# New measurement form: payment method as list + real tower/addendum selector + continue callback
...
s=rep(s,old,new,'measurement submit')
old="case 'measurement':content=shell(<>{!focusedContractId&&select('Contrato','contractId',contractOptions,true)}{input('Nº medição','measurementNumber','text',true)}{input('Competência','competence','month',true)}{input('Torre / Aditivo / Provisório','originLabel','text',true)}{input('Vencimento previsto','dueDate','date')}{input('Data prevista para pagamento','expectedPaymentDate','date')}{input('Forma de pagamento','paymentMethod')}{input('Observações','notes')}</>,'INSS, ISS e retenção técnica serão tratados na etapa Valores / Impostos e herdados do contrato.');break;"
new="case 'measurement':content=shell(<>{!focusedContractId&&select('Contrato','contractId',contractOptions,true)}{input('Nº medição','measurementNumber','text',true)}{input('Competência','competence','month',true)}{select('Torre / Aditivo','originKey',measurementOriginOptions,true)}{input('Vencimento previsto','dueDate','date')}{input('Data prevista para pagamento','expectedPaymentDate','date')}{select('Forma de pagamento','paymentMethod',paymentMethodOptions,true)}{input('Observações','notes')}</>,'INSS, ISS e retenção técnica serão tratados na etapa Valores / Impostos e herdados do contrato.');break;"
if old not in s:
    old="case 'measurement':content=shell(<>{!focusedContractId&&select('Contrato','contractId',contractOptions,true)}{input('Competência','competence','month',true)}{input('Nº medição','measurementNumber','text',true)}{input('Vencimento previsto','dueDate','date')}{input('Data prevista para pagamento','expectedPaymentDate','date')}{input('Forma de pagamento','paymentMethod')}{input('Torre / Aditivo','originLabel')}{input('Observações','notes')}</>,'INSS, ISS e retenção técnica serão carregados automaticamente conforme o contrato. Os dados financeiros acompanharão a medição até o contas a receber.');break;"
s=rep(s,old,new,'measurement form')
old="<Dialog open={kind!==null} title={kind?titles[kind]:'Engenharia'} description={kind?descriptions[kind]:'Operação de Engenharia'} loading={operations.state.busy} onClose={close} onBack={close} onConfirm={kind?()=>{void submit();}:undefined} confirmLabel={kind==='measurement'?'Salvar e continuar':'Salvar'}>"
...
if old not in s:
    old="<Dialog open={kind!==null} title={kind?titles[kind]:'Engenharia'} description={kind?descriptions[kind]:'Operação de Engenharia'} loading={operations.state.busy} onClose={close} onBack={close} onConfirm={kind?()=>{void submit();}:undefined} confirmLabel=\"Salvar\">"
s=rep(s,old,new,'measurement fullscreen dialog')
p.write_text(s)

# Contract workspace: after creating measurement, open selected origin in fullscreen measurement flow
...
# Fullscreen guided flow: accept the origin picked in New Measurement and open services directly
