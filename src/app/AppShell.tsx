...
export function AppShell({ session }: AppShellProps) {
  ...
  const allCompaniesBanks = <div className="app-company-sections app-company-sections--banks">{companies.map((company) => <section key={company.id} aria-label={`Bancos ${companyLabel(company)}`}><BanksPage company={company}/></section>)}</div>;
