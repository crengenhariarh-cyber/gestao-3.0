...
export function EngineeringContractWorkspace({section,scope,contract,onChanged,onNavigate}:Props){
  ...
  if(section==='resumo') content=<div className="engineering-contract-workspace__page engineering-contract-dashboard">
    ...
    if(section==='contrato'){
      ...
      const structureRows=selectedStructure?contractServices.map((service,index)=>({service,index,allocation:structureAllocations.find(item=>item.contractServiceId===service.id)})).filter(row=>match(row.service.description,row.service.unit,selectedStructure.name)&&matchService(row.service.description,row.service.unit,String(row.index+1),row.allocation?String(row.allocation.allocatedQuantity):null)):[];
      body=<>{toolbar('Adicionar serviço','contractService',{label:'Distribuir quantitativo',kind:'allocation'})}{sheetHead(contractServices.length,'Distribuições',String(allocations.length))}
        ...
        {selectedStructure&&<div className="engineering-sheet__detail-card"><div className="engineering-sheet__detail-head"><div><small>PLANILHA SELECIONADA</small><strong>{selectedStructure.name}</strong><span>{structureAllocations.length} serviço(s) com quantitativo definido</span></div><Button size="sm" onClick={()=>open('allocation')}>Editar quantitativo</Button></div><div className="engineering-sheet__live-filter"><span aria-hidden="true">⌕</span><input autoFocus value={serviceSearch} onChange={event=>setServiceSearch(event.target.value)} placeholder="Filtrar serviços enquanto digita: código, descrição, unidade ou quantitativo…" aria-label={`Filtrar serviços da ${selectedStructure.name}`}/>{serviceSearch&&<button type="button" onClick={()=>setServiceSearch('')} aria-label="Limpar filtro">×</button>}<small>{structureRows.length} de {contractServices.length}</small></div><div className="engineering-sheet__table-wrap engineering-sheet__table-wrap--detail"><table className="engineering-sheet__table engineering-sheet__table--services"><thead><tr><th>#</th><th>Serviço</th><th>Unidade</th><th>Valor unit.</th><th>Quantitativo</th><th>Ações</th></tr></thead><tbody>{structureRows.map(row=><tr key={row.service.id}><td>{String(row.index+1).padStart(3,'0')}</td><td><strong>{row.service.description}</strong></td><td>{row.service.unit}</td><td>{currency.format(row.service.unitPrice)}</td><td><strong>{row.allocation?quantity.format(row.allocation.allocatedQuantity):'—'}</strong></td><td><Button size="sm" variant="tertiary" onClick={()=>open('allocation')}>{row.allocation?'Editar quantitativo':'Definir quantitativo'}</Button></td></tr>)}</tbody></table>{structureRows.length===0&&emptyRow(serviceSearch?'Nenhum serviço corresponde ao filtro digitado.':'Nenhum serviço encontrado para esta estrutura.')}</div></div>}
