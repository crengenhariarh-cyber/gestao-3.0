...
export function EngineeringPage({company}:EngineeringPageProps){
  ...
  return <section className="engineering-overview" aria-labelledby="engineering-title">
    ...
    <div className="engineering-workspace" role="tabpanel" tabIndex={0}><div className="engineering-workspace__canvas">
      {activeTab==='contratos'&&<><div className="engineering-kpis engineering-kpis--contracts">
        <Card title="Valor contratado"><strong>{currency.format(contractUpdatedTotal)}</strong></Card>
        <Card title="Valor medido"><strong>{currency.format(contractMeasuredTotal)}</strong><span>{contractMeasuredPercent.toFixed(2)}% do contratado</span></Card>
        <Card title="Saldo contratual"><strong>{currency.format(contractBalanceTotal)}</strong></Card>
        <Card title="Contratos"><strong>{data.contracts.length}</strong><span>registros nesta empresa</span></Card>
      </div><Card className="engineering-primary-card" title="Contratos" description="Valor atualizado, medido e saldo">{data.contracts.length===0?empty:<div className="engineering-data-list engineering-data-list--contracts">{data.contracts.map(item=><div className="engineering-data-row" key={item.contractId}><div><strong>{item.contractNumber}</strong><span>{item.status}</span></div><div><span>{currency.format(item.updatedContractValue)}</span><span>Medido {currency.format(item.measuredNet)} · {item.measuredPercent.toFixed(2)}%</span><span>Saldo {currency.format(item.grossBalance)}</span></div></div>)}</div>}</Card></>}
      {activeTab==='medicoes'&&<><div className="engineering-kpis engineering-kpis--measurements">
        <Card title="Bruto medido"><strong>{currency.format(measurementGrossTotal)}</strong></Card>
        <Card title="Retenções"><strong>{currency.format(measurementRetainedTotal)}</strong></Card>
        <Card title="Líquido"><strong>{currency.format(measurementNetTotal)}</strong></Card>
        <Card title="Medições"><strong>{data.measurements.length}</strong><span>registros nesta empresa</span></Card>
