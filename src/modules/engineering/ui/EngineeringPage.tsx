...
export function EngineeringPage({company}:EngineeringPageProps) {
  ...
  const provisionals=data.provisionals;
  ...
  return <section className="engineering-overview" aria-labelledby="engineering-title">
    ...
    <div className="engineering-workspace" role="tabpanel" tabIndex={0}><div className="engineering-workspace__canvas">
      {activeTab==='contratos'&&<Card title="Contratos" description="Valor atualizado, medido e saldo">{contracts.length===0?empty:<div className="engineering-data-list">{contracts.map(item=><div className="engineering-data-row" key={item.contractId}><div><strong>{item.contractNumber}</strong><span>{item.status}</span></div><div><span>{currency.format(item.updatedContractValue)}</span><span>Medido {currency.format(item.measuredNet)} · {item.measuredPercent.toFixed(2)}%</span><span>Saldo {currency.format(item.grossBalance)}</span></div></div>)}</div>}</Card>}
      {activeTab==='medicoes'&&<Card title="Medições" description="Competência, retenções e valor líquido">{measurements.length===0?empty:<div className="engineering-data-list">{measurements.map(item=><div className="engineering-data-row" key={item.measurementId}><div><strong>{item.competence}</strong><span>{item.status}</span></div><div><span>Bruto {currency.format(item.grossAmount)}</span><span>Retido {currency.format(item.retainedAmount)}</span><span>Líquido {currency.format(item.netAmount)}</span></div></div>)}</div>}</Card>}
      {activeTab==='producao'&&<Card title="Produção" description="Produção registrada por vínculo e competência">{production.length===0?empty:<div className="engineering-data-list">{production.map((item,index)=><div className="engineering-data-row" key={`${item.employmentContractId}-${item.competence}-${index}`}><div><strong>{item.competence}</strong><span>Vínculo {item.employmentContractId.slice(0,8)}</span></div><div><span>Qtd. {item.executedQuantity}</span><span>{currency.format(item.productionValue)}</span></div></div>)}</div>}</Card>}
      {activeTab==='aditivos'&&<Card title="Aditivos" description="Aditivos vinculados aos contratos">{addenda.length===0?empty:<div className="engineering-data-list">{addenda.map(item=><div className="engineering-data-row" key={item.id}><div><strong>{item.addendumNumber}</strong><span>{item.addendumType} · {item.status}</span></div><div><span>{item.statedValue===null?'Valor por linhas':currency.format(item.statedValue)}</span></div></div>)}</div>}</Card>}
      {activeTab==='provisorios'&&<Card title="Provisórios" description="Negociações editáveis antes da conversão">{provisionals.length===0?empty:<div className="engineering-data-list">{provisionals.map(item=><div className="engineering-data-row" key={item.id}><div><strong>{item.provisionalNumber}</strong><span>{item.title??'Sem título'}</span></div><div><span>{item.status}</span><span>{item.clientName??'Cliente não informado'}</span></div></div>)}</div>}</Card>}
    </div></div>
  </section>;
}
