...
export function EngineeringPage({companies,initialCompanyId}:EngineeringPageProps){
  ...
  const contractEntries=[
    ...data.contracts.map(item=>({kind:'contract' as const,id:item.contractId,companyId:item.companyId,title:item.workName,subtitle:`${item.clientName??companyLabel(companies.find(c=>c.id===item.companyId)??companies[0]!)} · ${item.contractNumber}`,status:item.status,measuredPercent:item.measuredPercent,contract:item,negotiatedValue:null})),
    ...data.provisionals.map(item=>({kind:'provisional' as const,id:item.id,companyId:item.companyId,title:item.clientName??item.title??'Provisório',subtitle:item.provisionalNumber,status:item.status,measuredPercent:null,contract:null,negotiatedValue:item.negotiatedValue})),
  ];
      ...
      <div className="engineering-contract-list">
        {filteredContracts.length===0?empty:filteredContracts.map(item=><Card className="engineering-contract-card" key={`${item.kind}-${item.id}`}>
          <Button variant="tertiary" className="engineering-contract-card__open" onClick={()=>{if(item.kind==='contract'&&item.contract)setSelectedContract(item.contract);else{setSelectedCompanyId(item.companyId);setActiveTab('provisorios');}}}>
            ...
            {item.negotiatedValue!==null&&<div className="engineering-contract-card__values engineering-contract-card__values--negotiation"><span>Valor em negociação <strong>{currency.format(item.negotiatedValue)}</strong></span></div>}
          </Button>
        </Card>)}
      </div>
    </section>}
