...
export function GuidedMeasurementFlow({scope,contractId,initialMeasurementId='',initialOriginId='',draftHeader=null,onDraftPersisted,onChanged,onClose}:Props){
  ...
  const previousLines=useMemo(()=>targetLines.filter(line=>line.measurementId!==measurementId&&['closed','approved'].includes(line.measurementStatus)),[targetLines,measurementId]);
  ...
  const filteredStages=useMemo(()=>{if(!model||!origin)return [];return stages.map((item,index)=>({item,index})).filter(({item})=>{
    ...
    const previous=model.lines.filter(line=>line.measurementId!==measurementId&&['closed','approved'].includes(line.measurementStatus)&&`${line.targetKind}:${line.targetId}`===key).reduce((sum,line)=>sum+line.measuredQuantity,0);
