...
export function GuidedMeasurementFlow({scope,contractId,initialMeasurementId='',initialOriginId='',draftHeader=null,onDraftPersisted,onChanged,onClose}:Props){
  ...
  const [pickerOpen,setPickerOpen]=useState(false); const [unitPickerOpen,setUnitPickerOpen]=useState(false); const [serviceSearch,setServiceSearch]=useState(''); const [selectedUnits,setSelectedUnits]=useState<string[]>([]); const [search,setSearch]=useState(''); const [manualQuantity,setManualQuantity]=useState('');
  ...
  useEffect(()=>{if(draftMode&&!initialMeasurementId){if(measurementId)setMeasurementId('');return;}if(initialMeasurementId&&drafts.some(item=>item.id===initialMeasurementId)){setMeasurementId(initialMeasurementId);return;}if(measurementId&&drafts.some(item=>item.id===measurementId))return;setMeasurementId('');},[draftMode,drafts,measurementId,initialMeasurementId]);
  ...
  async function openOrigin(nextOriginId:string){if(!nextOriginId)return;if(measurementId){try{await saveHeader();}catch{return;}}setOriginId(nextOriginId);setServiceIndex(0);setServiceSearch('');setUnitPickerOpen(false);setError(null);setPickerOpen(true);}
  ...
  async function confirmSelection(){
    ...
    setSaving(true);setError(null);try{await replaceMeasurementParityStage(scope,{measurementId:activeMeasurementId,targetKind:stage.targetKind,targetId:stage.targetId,quantity:effectiveQuantity,references:refs.length?selectedUnits:[],originName:origin.name,legacyServiceId:stage.legacyServiceId});await reload();closeUnitPicker();onChanged();}catch(cause){setError(cause instanceof Error?cause.message:'Não foi possível confirmar a seleção.');}finally{setSaving(false);}
  }
  ...
  return <Dialog open variant="measurement-fullscreen" title="Medição" description="Selecione a origem e lance os serviços sem sair desta medição." onClose={closeFlow} onBack={closeFlow}>
    ...
    {pickerOpen&&origin&&<div className="guided-measurement-picker" role="dialog" aria-modal="true" aria-label={`Serviços de ${originLabel(origin)}`}><div className="guided-measurement-picker__panel guided-measurement-origin-sheet">
