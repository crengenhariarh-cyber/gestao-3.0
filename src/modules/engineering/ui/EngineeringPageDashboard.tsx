  ...
  return <>
    ...
    {data&&<section className="engineering-dashboard" aria-label="Dashboard da Engenharia">
      ...
      <div className="engineering-dashboard__main">
        <Card className="engineering-dashboard__progress" title="Execução geral"><div className="engineering-dashboard__donut" style={{'--progress':`${percent*3.6}deg`} as React.CSSProperties}><strong>{percent.toFixed(1)}%</strong><span>Executado</span></div><div className="engineering-dashboard__legend"><span><i className="is-measured"/>Medido <strong>{currency.format(measured)}</strong></span><span><i className="is-balance"/>Saldo <strong>{currency.format(balance)}</strong></span><span><i className="is-contract"/>Contratado <strong>{currency.format(contracted)}</strong></span></div></Card>
