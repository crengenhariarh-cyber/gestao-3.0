...
interface State {
  busy: boolean;
  data: EngineeringOperationalSnapshot | null;
  errorMessage: string | null;
  successMessage: string | null;
}

function messageFrom(error: unknown): string {
  return error instanceof Error && error.message ? error.message : 'Não foi possível concluir a operação de Engenharia.';
}
...
export function useEngineeringOperations(scope: EngineeringScope) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const data = await repository.getSnapshot(scope);
  ...
  const execute = useCallback(async (action: () => Promise<void>, successMessage: string) => {
    ...
    try {
      await action();
      const data = await repository.getSnapshot(scope);
      setState({ busy: false, data, errorMessage: null, successMessage });
    } catch (error) {
      setState((current) => ({ ...current, busy: false, errorMessage: messageFrom(error), successMessage: null }));
      throw error;
    }
  }, [repository, scope]);
