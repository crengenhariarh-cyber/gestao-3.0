...
export function EngineeringOperationsPanel({activeTab,scope,onChanged,actionsMode='default',focusedContractId=null}:Props){
  ...
  const defaults:Record<Exclude<Kind,null>,Record<string,string>>={
    ...
    contract:{workId:'',contractNumber:'',clientName:'',signedAt:'',startDate:'',endDate:'',notes:''},contractStatus:{contractId:'',status:'active'},
  ...
  switch(kind){
    ...
    case 'contract':content=<div className="engineering-form-grid">{select('Obra','workId',workOptions,true)}{input('Número','contractNumber','text',true)}{input('Cliente','clientName')}{input('Assinatura','signedAt','date')}{input('Início','startDate','date')}{input('Fim','endDate','date')}</div>;break;
    ...
    case 'measurement':content=<div className="engineering-form-grid">{select('Contrato','contractId',contractOptions,true)}{input('Competência','competence','month',true)}</div>;break;
