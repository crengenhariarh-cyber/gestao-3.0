...
export interface EngineeringProductionSnapshot {
  ...
  entries:EngineeringProductionEntryView[];
}
...
type StructureRow={id:string;name:string};
type ServiceRow={id:string;name:string};
type EmployeeRow={id:string;employees:{full_name:string}[]};

export async function loadEngineeringProduction(scope:EngineeringProductionScope,workId:string):Promise<EngineeringProductionSnapshot>{
  ...
  const structureNames=new Map((structures.data??[]).map(item=>[item.id,item.name]));
  const serviceNames=new Map((services.data??[]).map(item=>[item.id,item.name]));
  const employeeNames=new Map((employees.data??[]).map(item=>[item.id,item.employees[0]?.full_name??'Colaborador']));
  return {
    periods:periodRows.map(item=>({id:item.id,workId:item.work_id,competence:item.competence,status:item.status})),
    entries:(entries.data??[]).map(item=>({
      id:item.id,
      periodId:item.production_period_id,
      employmentContractId:item.employment_contract_id,
      employeeName:employeeNames.get(item.employment_contract_id)??'Colaborador',
      structureId:item.structure_id,
      structureName:structureNames.get(item.structure_id)??'Estrutura',
      serviceId:item.service_id,
      serviceName:item.service_id?serviceNames.get(item.service_id)??'Serviço':'Serviço legado / provisório',
      productionDate:item.production_date,
      executedQuantity:Number(item.executed_quantity),
      unitValue:item.unit_value===null?null:Number(item.unit_value),
      productionValue:item.production_value===null?null:Number(item.production_value),
      notes:item.notes,
    })),
  };
}
