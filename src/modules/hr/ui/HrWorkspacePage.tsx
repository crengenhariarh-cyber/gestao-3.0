...
import type { HrOperationalSnapshot } from '../application/HrOperationsRepository';
...
type CompanyData = {
  ...
  operations: HrOperationalSnapshot;
  overview: Awaited<ReturnType<ReturnType<typeof getHrBudgetRepository>['getOverview']>>;
};
...
export function HrWorkspacePage({ companies, initialCompanyId }: { companies: readonly CompanySummary[]; initialCompanyId?: string }) {
  ...
  const [companyFilter, setCompanyFilter] = useState(() => initialCompanyId && companies.some((item) => item.id === initialCompanyId) ? initialCompanyId : ALL_HR_COMPANIES);
  ...
  useEffect(() => {
    if (initialCompanyId && companies.some((item) => item.id === initialCompanyId)) setCompanyFilter(initialCompanyId);
    else if (initialCompanyId === undefined) setCompanyFilter(ALL_HR_COMPANIES);
  }, [companies, initialCompanyId]);

  const visibleCompanies = useMemo(() => companyFilter === ALL_HR_COMPANIES ? companies : companies.filter((item) => item.id === companyFilter), [companies, companyFilter]);

  useEffect(() => {
    let cancelled = false;
    const operationsRepository = getHrOperationsRepository();
    const budgetRepository = getHrBudgetRepository();
    setLoading(true);
    setErrorMessage(null);

    void Promise.all(visibleCompanies.map(async (company) => {
      const scope = { tenantId: company.tenantId, companyId: company.id };
      const [operations, overview] = await Promise.all([
        operationsRepository.getSnapshot(scope, competenceMonth),
        budgetRepository.getOverview({ ...scope, competenceMonth, year: Number(competenceMonth.slice(0, 4)) }),
      ]);
      return { company, operations, overview } satisfies CompanyData;
    }))
      .then((data) => { if (!cancelled) setCompanyData(data); })
      .catch(() => { if (!cancelled) { setCompanyData([]); setErrorMessage('Não foi possível carregar os dados do RH para o filtro selecionado.'); } })
      .finally(() => { if (!cancelled) setLoading(false); });

    return () => { cancelled = true; };
  }, [visibleCompanies, competenceMonth]);

  const employees = companyData.flatMap(({ company, operations }) => operations.employees.map((item) => ({ ...item, companyId: company.id, companyName: companyLabel(company) })));
  ...
  const works = new Set(activeEmployees.map((item) => `${item.companyId}:${item.costCenterId ?? ''}`).filter((item) => !item.endsWith(':'))).size;
  const bankHours = activeEmployees.filter((item) => item.bankHoursEnabled).length;
  const eventTotal = companyData.reduce((total, item) => total + item.operations.payrollEvents.filter((event) => event.status === 'active').reduce((sum, event) => sum + event.amount, 0), 0);

  const companyOptions = [
    ...(companies.length > 1 ? [{ value: ALL_HR_COMPANIES, label: 'Todas as empresas' }] : []),
    ...companies.map((company) => ({ value: company.id, label: companyLabel(company) })),
  ];
  const actionCompanyOptions = companyFilter === ALL_HR_COMPANIES
  ...
  return <section className="hr-workspace">
    ...
    {!loading && !errorMessage && tab === 'dashboard' && <div className="hr-workspace__content">
      <Card title="Painel financeiro do RH" description={`${activeEmployees.length} colaboradores ativos · ${scopeLabel}`}><div className="hr-workspace__primary-total"><span>Total da competência</span><strong>{money.format(salaryRealized)}</strong><small>Previsto {money.format(salaryPlanned)}</small></div></Card>
