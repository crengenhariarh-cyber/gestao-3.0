  ...
  const loadReferences = useCallback(async (): Promise<FinanceReferenceData> => {
    ...
    try {
      const [categories, costCenters, accounts, cards, statements, installmentBalances, transfers, recurrences] = await Promise.all([
        repositories.registries.listCategories(scope), repositories.registries.listCostCenters(scope), repositories.registries.listAccounts(scope), repositories.cards.listCards(scope), repositories.cards.listStatements(scope), repositories.settlements.listBalances(scope), repositories.accounts.listTransfers(scope), repositories.recurrences.list(scope),
      ]);
