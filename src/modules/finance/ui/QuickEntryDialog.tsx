...
function QuickEntryForm({ company, onClose }: QuickEntryFormProps) {
  ...
  async function launch(keepData: boolean) {
    ...
    if (!form.description.trim() || !Number.isFinite(amount) || amount <= 0 || !form.categoryId) return;
    ...
    try {
      if (form.launchType === 'recurring') {
        ...
        if (!form.cardId || form.entryType !== 'expense') return;
  ...
  if (!references && operations.state.busy) return <LoadingState label="Carregando dados do lançamento…" />;
  ...
  return <>
    ...
    <div className="quick-entry">
      ...
      <div className="quick-entry__grid">
        ...
        <Input label="Empresa" value={company.tradeName ?? company.legalName} disabled />
    ...
    <div className="quick-entry__footer-actions">
...
export function QuickEntryDialog({ open, companies, initialCompanyId = '', onClose }: QuickEntryDialogProps) {
  ...
  return <Dialog
    ...
    description="Preencha os dados principais. Campos adicionais aparecem conforme o tipo e a forma de pagamento."
    onClose={onClose}
    onBack={onClose}
    footer={company ? <QuickEntryFooterNotice /> : undefined}
  >
    {companies.length > 1 && <div className="quick-entry__company"><Select label="Empresa do lançamento" value={companyId} options={companyOptions} onChange={(event) => setCompanyId(event.target.value)} /></div>}
    {company ? <QuickEntryForm key={company.id} company={company} onClose={onClose} /> : <Feedback tone="danger" title="Empresa obrigatória" message="Selecione uma empresa para continuar." />}
  </Dialog>;
}

function QuickEntryFooterNotice() {
  return <span className="quick-entry__footer-hint">Use os botões de lançamento no fim do formulário.</span>;
}
