...
export function FinancePage({ company }: FinancePageProps) {
  ...
  const overview = useFinanceOverview({ tenantId: company.tenantId, companyId: company.id });
  ...
  return (
    <section className="finance-overview" aria-labelledby="finance-title">
      ...
      {activeTab === 'cartoes' && <div role="tabpanel"><Card title="Cartões" description="Limite total, comprometido e disponível">{data.cardLimits.length === 0 ? <p className="ui-muted">Nenhum cartão cadastrado.</p> : <div className="finance-list">{data.cardLimits.map((card) => <div className="finance-list__group" key={card.cardId}><strong>{card.name}</strong><div className="finance-list__row"><span>Limite</span><span>{currency.format(card.creditLimit)}</span></div><div className="finance-list__row"><span>Comprometido</span><span>{currency.format(card.committedAmount)}</span></div><div className="finance-list__row"><span>Disponível</span><span>{currency.format(card.availableLimit)}</span></div></div>)}</div>}</Card></div>}
    </section>
  );
}
