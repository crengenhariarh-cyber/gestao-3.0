...
export function FinancePage({ company, allowDirectAction = false }: FinancePageProps) {
  ...
  const totalCardLimit = activeCards.reduce((sum, card) => sum + card.creditLimit, 0);
  const totalCardAvailable = activeCards.reduce((sum, card) => {
    const limit = data.cardLimits.find((item) => item.cardId === card.id);
    return sum + (limit?.availableLimit ?? card.creditLimit);
  }, 0);
  const totalCardUsed = Math.max(0, totalCardLimit - totalCardAvailable);
  const totalOpenStatements = openStatements.reduce((sum, item) => sum + item.remainingAmount, 0);
  ...
  return <section className="finance-overview" aria-labelledby="finance-title">
    ...
    {activeTab === 'cartoes' && <div className="finance-section finance-cards-panel" role="tabpanel">
      <div className="finance-overview__cards finance-cards-summary">
        <Card title="Limite total"><strong className="balance-card__value">{currency.format(totalCardLimit)}</strong></Card>
        <Card title="Limite utilizado"><strong className="balance-card__value">{currency.format(totalCardUsed)}</strong></Card>
        <Card title="Limite disponível"><strong className="balance-card__value">{currency.format(totalCardAvailable)}</strong></Card>
        <Card title="Faturas em aberto"><strong className="balance-card__value">{currency.format(totalOpenStatements)}</strong></Card>
      </div>
      <Card className="finance-cards-list" title="Meus cartões" description="Limite, disponibilidade, fechamento e vencimento de cada cartão">
        {(references?.cards.length ?? 0) === 0 ? <p className="ui-muted">Nenhum cartão cadastrado.</p> : <div className="finance-list">{references?.cards.map((card) => { const limit = data.cardLimits.find((item) => item.cardId === card.id); const available = limit?.availableLimit ?? card.creditLimit; const used = Math.max(0, card.creditLimit - available); return <div className="finance-list__group finance-card-item" key={card.id}><div className="finance-list__row"><strong>{card.name}{card.lastFour ? ` · ${card.lastFour}` : ''}</strong><span>{card.status === 'active' ? 'Ativo' : 'Inativo'}</span></div><div className="finance-list__row"><span>Limite {currency.format(card.creditLimit)}</span><strong>Disponível {currency.format(available)}</strong></div><div className="finance-list__row"><span>Utilizado {currency.format(used)}</span><span>Fecha dia {card.closingDay} · vence dia {card.dueDay}</span></div><div className="finance-actions"><Button size="sm" variant="secondary" onClick={() => open('card', { id: card.id, name: card.name, lastFour: card.lastFour ?? '', creditLimit: String(card.creditLimit), closingDay: String(card.closingDay), dueDay: String(card.dueDay), defaultPaymentAccountId: card.defaultPaymentAccountId ?? '', status: card.status })}>Editar</Button><Button size="sm" variant="tertiary" disabled={operations.state.busy} onClick={() => { void complete(() => operations.updateCard({ id: card.id, name: card.name, lastFour: card.lastFour, creditLimit: card.creditLimit, closingDay: card.closingDay, dueDay: card.dueDay, defaultPaymentAccountId: card.defaultPaymentAccountId, status: card.status === 'active' ? 'inactive' : 'active' })).catch(() => undefined); }}>{card.status === 'active' ? 'Inativar' : 'Ativar'}</Button></div></div>; })}</div>}
      </Card>
      <Card className="finance-card-installments" title="Parcelas dos cartões" description="Toda compra parcelada mantém identificação X/Y">{(references?.cardInstallments.length ?? 0) === 0 ? <p className="ui-muted">Nenhuma parcela de cartão registrada.</p> : <div className="finance-list">{references?.cardInstallments.map((item) => <div className="finance-list__row" key={item.id}><span>{references?.cards.find((card) => card.id === item.cardId)?.name ?? 'Cartão'} · Parcela {item.installmentNumber}/{item.installmentCount} · {formatMonth(item.statementMonth)}</span><strong>{currency.format(item.amount)}</strong></div>)}</div>}</Card>
      <Card className="finance-card-statements" title="Faturas" description="Valores, saldo e status de pagamento">{(references?.statements.length ?? 0) === 0 ? <p className="ui-muted">Nenhuma fatura registrada.</p> : <div className="finance-list">{references?.statements.map((item) => <div className="finance-list__group" key={item.statementId}><div className="finance-list__row"><strong>{references?.cards.find((card) => card.id === item.cardId)?.name ?? 'Cartão'}</strong><strong>{formatMonth(item.statementMonth)}</strong></div><div className="finance-list__row"><span>{item.paymentStatus}</span><span>Fatura {currency.format(item.statementAmount)} · saldo {currency.format(item.remainingAmount)}</span></div></div>)}</div>}</Card>
    </div>}
