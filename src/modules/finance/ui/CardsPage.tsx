...
export function CardsPage({ companies, availableCompanies = companies }: { companies: readonly CompanySummary[]; availableCompanies?: readonly CompanySummary[] }) {
  ...
  const [activityForm, setActivityForm] = useState({ date: '', description: '', amount: '' });
  ...
  function openActivityEdit(item: CardStatementActivity) { setSelectedActivity(item); setActivityForm({ date: item.activityDate, description: item.activityType === 'payment' ? item.counterpartyName ?? '' : item.description, amount: String(item.sourceTotalAmount) }); setDialog('activityEdit'); }
  ...
  async function saveActivity() {
    ...
    try {
      ...
      if (selectedActivity.activityType === 'purchase') await repositories.cards.updatePurchase({ ...scope, transactionId: selectedActivity.sourceId, purchaseDate: activityForm.date, description: activityForm.description, totalAmount: money(activityForm.amount) });
