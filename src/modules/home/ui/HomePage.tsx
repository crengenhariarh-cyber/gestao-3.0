...
function accumulate(values: number[]): void { for (let i = 1; i < values.length; i += 1) values[i] = (values[i] ?? 0) + (values[i - 1] ?? 0); }
...
export function HomePage({ companies }: HomePageProps) {
  ...
  data.entries.forEach((item) => {
    const index = Math.max(0, Math.min(30, dayOf(item.dueDate) - 1));
    balanceSeries[index] = (balanceSeries[index] ?? 0) + (item.entryType === 'income' ? item.amount : -item.amount);
  });
  accumulate(balanceSeries);
  const projectedSeries = balanceSeries.map((value) => data.bankBalance + value);
  ...
  const costCenterBudgets = data.budgets.filter((item) => item.costCenterId !== null).slice(0, 6);
  ...
  return (
    <section className="home-sequence" aria-label="Painel inicial">
      <div className="home-sequence__toolbar">
        ...
        <Button size="sm" onClick={() => go('/financeiro')}>Atualizar</Button>
      </div>

      <Card>
        <div className="home-actions">
          <Button onClick={() => go('/financeiro')}>＋ Lançamento</Button>
          <Button variant="secondary" onClick={() => go('/financeiro')}>Contas do mês</Button>
        </div>
      </Card>
      ...
      <Card>
        <div className="balance-card">
          ...
          <svg className="balance-chart" viewBox="0 0 620 170" role="img" aria-label="Evolução do saldo nos últimos 30 dias">
            ...
            <polyline className="balance-chart__line" points={polyline(projectedSeries)} />
          </svg>
      ...
      <Card title="Orçamento do mês" actions={<Button size="sm" onClick={() => go('/rh')}>＋ Definir orçamento</Button>}>
        <div className="budget-list">
          {costCenterBudgets.length === 0 ? <span className="ui-muted">Nenhum orçamento por obra/centro de custo nesta competência.</span> : costCenterBudgets.map((item) => {
            const available = item.plannedTotal - item.realizedTotal;
            return <button className="budget-item" key={`${item.companyId}-${item.costCenterId ?? 'geral'}`} onClick={() => go('/rh')}><div><strong>{item.costCenterName ?? 'Geral'}</strong><small>{companies.length > 1 ? item.companyName : 'Orçamento mensal'}</small></div><div><strong>{money(available)}</strong><small>disponível</small></div><span aria-hidden="true">›</span></button>;
          })}
        </div>
      </Card>

      <Card title="Resumo diário" actions={<Button size="sm" variant="secondary" onClick={() => go('/financeiro')}>Planejar</Button>}>
