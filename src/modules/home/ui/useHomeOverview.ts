...
export function useHomeOverview(companies:readonly CompanySummary[],refreshToken=0):HomeOverviewState{
 ...
 useEffect(()=>{
  ...
  void Promise.all(companies.map(async company=>{const scope={tenantId:company.tenantId,companyId:company.id}; const [summary,balances,movements,entries,budget,cardLimits]=await Promise.all([
   finance.monthly.summarize({...scope,competenceFrom:month,competenceTo:month}),finance.accounts.listBalances(scope),finance.accounts.listMovements(scope,isoDate(movementFrom),isoDate(movementTo)),finance.entries.list(scope),hr.getOverview({...scope,competenceMonth:hrMonth,year:Number(hrMonth.slice(0,4))}),finance.cards.listLimits(scope)]); return {company,summary,balances,movements,entries,budget,cardLimits};}))
  .then(results=>{if(cancelled)return; let bankBalance=0,incomePlanned=0,incomeRealized=0,expensePlanned=0,expenseRealized=0; const entries:HomeEntry[]=[],balanceMovements:HomeBalanceMovement[]=[],budgets:HomeBudgetItem[]=[],bankAccounts:HomeBankAccount[]=[],cards:HomeCard[]=[];
   results.forEach(({company,summary,balances,movements,entries:companyEntries,budget,cardLimits})=>{const label=companyName(company); const dashboardAccounts=balances.filter(i=>i.status==='active'&&i.includeInDashboard); const ids=new Set(dashboardAccounts.map(i=>i.accountId));
    ...
    budget.monthlyBudget.filter(i=>i.costCenterId!==null&&i.plannedManual!==0).forEach(i=>budgets.push({companyId:company.id,companyName:label,costCenterId:i.costCenterId,costCenterName:i.costCenterName,plannedTotal:i.plannedTotal,realizedTotal:i.realizedTotal}));
