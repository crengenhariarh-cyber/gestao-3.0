...
export function useHomeOverview(companies:readonly CompanySummary[],refreshToken=0):HomeOverviewState{
 ...
 useEffect(()=>{
  ...
  void Promise.all(companies.map(async company=>{const scope={tenantId:company.tenantId,companyId:company.id}; const [summary,balances,movements,entries,budget,operational,cardLimits,limitConsumptionResult]=await Promise.all([
   finance.monthly.summarize({...scope,competenceFrom:month,competenceTo:month}),finance.accounts.listBalances(scope),finance.accounts.listMovements(scope,isoDate(movementFrom),isoDate(movementTo)),finance.entries.list(scope),hr.getOverview({...scope,competenceMonth:hrMonth,year:Number(hrMonth.slice(0,4))}),hrOperations.getSnapshot(scope,hrMonth),finance.cards.listLimits(scope),supabase.from('budget_limit_control').select('limit_id,consumed_amount,remaining_amount,consumed_percent').eq('tenant_id',scope.tenantId).eq('company_id',scope.companyId).eq('competence_month',hrMonth).returns<BudgetLimitConsumptionRow[]>()]);
   if(limitConsumptionResult.error)throw limitConsumptionResult.error;
   return {company,summary,balances,movements,entries,budget,operational,cardLimits,limitConsumption:limitConsumptionResult.data??[]};}))
