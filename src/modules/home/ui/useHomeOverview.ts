...
export interface HomeEntry { tenantId:string; companyId:string; installmentId:string; sourceKind:'financial_installment'|'card_statement'; entryType:'income'|'expense'; description:string; counterpartyName:string|null; installmentNumber:number; installmentCount:number; dueDate:string; amount:number; paymentStatus:string; companyName:string; }
 ...
 useEffect(()=>{
  ...
  .then(results=>{if(cancelled)return; let bankBalance=0,incomePlanned=0,incomeRealized=0,expensePlanned=0,expenseRealized=0; const entries:HomeEntry[]=[],balanceMovements:HomeBalanceMovement[]=[],budgets:HomeBudgetItem[]=[],bankAccounts:HomeBankAccount[]=[],cards:HomeCard[]=[];
   results.forEach(({company,summary,balances,movements,budget,operational,cardLimits,limitConsumption,planning})=>{const label=companyName(company); const dashboardAccounts=balances.filter(i=>i.status==='active'&&i.includeInDashboard); const ids=new Set(dashboardAccounts.map(i=>i.accountId)); const consumptionByLimit=new Map(limitConsumption.map(row=>[row.limit_id,row]));
    ...
    planning.forEach(i=>entries.push({tenantId:company.tenantId,companyId:company.id,installmentId:i.item_key,sourceKind:i.source_kind,entryType:i.entry_type,description:i.description,counterpartyName:i.counterparty_name,installmentNumber:i.installment_number,installmentCount:i.installment_count,dueDate:i.due_date,amount:Number(i.amount),paymentStatus:i.payment_status,companyName:label}));
