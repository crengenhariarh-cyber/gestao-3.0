...
export interface HomeBudgetLimit { id:string; categoryName:string|null; limitAmount:number; warningPercent:number; }
...
export interface HomeBankAccount { companyId:string; companyName:string; accountId:string; name:string; bankInstitution:BankInstitution|null; currentBalance:number; }
export interface HomeCard { companyId:string; companyName:string; cardId:string; name:string; creditLimit:number; committedAmount:number; availableLimit:number; }
...
export function useHomeOverview(companies:readonly CompanySummary[],refreshToken=0):HomeOverviewState{
 ...
 useEffect(()=>{
  ...
  void Promise.all(companies.map(async company=>{const scope={tenantId:company.tenantId,companyId:company.id}; const [summary,balances,movements,entries,budget,operational,cardLimits]=await Promise.all([
   finance.monthly.summarize({...scope,competenceFrom:month,competenceTo:month}),finance.accounts.listBalances(scope),finance.accounts.listMovements(scope,isoDate(movementFrom),isoDate(movementTo)),finance.entries.list(scope),hr.getOverview({...scope,competenceMonth:hrMonth,year:Number(hrMonth.slice(0,4))}),hrOperations.getSnapshot(scope,hrMonth),finance.cards.listLimits(scope)]); return {company,summary,balances,movements,entries,budget,operational,cardLimits};}))
  .then(results=>{if(cancelled)return; let bankBalance=0,incomePlanned=0,incomeRealized=0,expensePlanned=0,expenseRealized=0; const entries:HomeEntry[]=[],balanceMovements:HomeBalanceMovement[]=[],budgets:HomeBudgetItem[]=[],bankAccounts:HomeBankAccount[]=[],cards:HomeCard[]=[];
   results.forEach(({company,summary,balances,movements,entries:companyEntries,budget,operational,cardLimits})=>{const label=companyName(company); const dashboardAccounts=balances.filter(i=>i.status==='active'&&i.includeInDashboard); const ids=new Set(dashboardAccounts.map(i=>i.accountId));
    bankBalance+=dashboardAccounts.reduce((t,i)=>t+i.currentBalance,0); dashboardAccounts.forEach(i=>bankAccounts.push({companyId:company.id,companyName:label,accountId:i.accountId,name:i.name,bankInstitution:i.bankInstitution,currentBalance:i.currentBalance}));
    ...
    budget.monthlyBudget.filter(i=>i.costCenterId!==null&&i.plannedManual!==0).forEach(i=>{const limits=operational.budgetLimits.filter(l=>l.status==='active'&&l.costCenterId===i.costCenterId).map(l=>({id:l.id,categoryName:l.categoryName,limitAmount:l.limitAmount,warningPercent:l.warningPercent})); budgets.push({companyId:company.id,companyName:label,costCenterId:i.costCenterId,costCenterName:i.costCenterName,plannedTotal:i.plannedTotal,realizedTotal:i.realizedTotal,limitTotal:limits.reduce((t,l)=>t+l.limitAmount,0),limits});});
 ...
 },[finance,hr,hrOperations,companyKey,refreshToken,companies]); return state;
}
