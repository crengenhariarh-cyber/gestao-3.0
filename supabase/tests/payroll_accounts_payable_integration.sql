...
insert into public.payroll_closings(id,tenant_id,company_id,employment_contract_id,competence_month,base_salary_snapshot,events_credit_snapshot,events_debit_snapshot,gross_snapshot,net_before_statutory_snapshot,idempotency_key,closed_by) values
...
insert into public.payroll_statutory_calculations(id,tenant_id,company_id,payroll_closing_id,competence_month,inss_base,inss_amount,irrf_taxable_income,irrf_deduction_method,irrf_deduction_amount,irrf_base,irrf_before_reduction,irrf_reduction,irrf_amount,fgts_base,fgts_rate,fgts_amount,ruleset,calculated_by) values
...
select public.configure_payroll_finance('29000000-0000-0000-0000-000000000001','39000000-0000-0000-0000-000000000001','49000000-0000-0000-0000-000000000001','49000000-0000-0000-0000-000000000002','49000000-0000-0000-0000-000000000003','49000000-0000-0000-0000-000000000004');

do $$ begin
  begin
    perform 1 from public.sync_payroll_accounts_payable('29000000-0000-0000-0000-000000000001','39000000-0000-0000-0000-000000000001','2026-09-01','2026-10-05','2026-10-20','2026-10-20','2026-10-20');
...
set local role authenticated;
select set_config('request.jwt.claim.sub','19000000-0000-0000-0000-000000000001',true);
select * from public.sync_payroll_accounts_payable('29000000-0000-0000-0000-000000000001','39000000-0000-0000-0000-000000000001','2026-09-01','2026-10-05','2026-10-20','2026-10-20','2026-10-20');
select * from public.sync_payroll_accounts_payable('29000000-0000-0000-0000-000000000001','39000000-0000-0000-0000-000000000001','2026-09-01','2026-10-05','2026-10-20','2026-10-20','2026-10-20');
reset role;

do $$ declare e integer; l integer; s numeric; f numeric; i numeric; r numeric;
begin
  ...
  if exists(select 1 from public.financial_installments where tenant_id='29000000-0000-0000-0000-000000000001' and company_id='39000000-0000-0000-0000-000000000001' and competence_month<>'2026-09-01') then raise exception 'payroll installment competence mismatch'; end if;
end $$;
