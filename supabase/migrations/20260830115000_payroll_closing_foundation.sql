...
create table public.payroll_closing_event_snapshots (
  ...
  constraint payroll_closing_event_event_fk foreign key (tenant_id, company_id, payroll_event_id)
    references public.payroll_events(tenant_id, company_id, id) on delete restrict,
...
create policy payroll_closings_select_authorized on public.payroll_closings for select to authenticated
using (app_private.can_access_company(tenant_id, company_id));
create policy payroll_closing_events_select_authorized on public.payroll_closing_event_snapshots for select to authenticated
using (app_private.can_access_company(tenant_id, company_id));
...
begin
  ...
  if found then
    if v_existing.employment_contract_id<>p_employment_contract_id or v_existing.competence_month<>p_competence_month then
  ...
  select
    coalesce(sum(case when event_type in ('benefit','overtime','positive_adjustment') then amount else 0 end),0),
    coalesce(sum(case when event_type in ('advance','absence','dsr','negative_adjustment') then amount else 0 end),0)
  into v_credit,v_debit
  from public.payroll_events
  where tenant_id=p_tenant_id and company_id=p_company_id and employment_contract_id=p_employment_contract_id
    and competence_month=p_competence_month;

  insert into public.payroll_closings(tenant_id,company_id,employment_contract_id,competence_month,base_salary_snapshot,events_credit_snapshot,events_debit_snapshot,gross_snapshot,net_before_statutory_snapshot,idempotency_key,closed_by)
  values(p_tenant_id,p_company_id,p_employment_contract_id,p_competence_month,v_salary,v_credit,v_debit,v_salary+v_credit,v_salary+v_credit-v_debit,p_idempotency_key,v_user_id)
  returning id into v_closing_id;

  insert into public.payroll_closing_event_snapshots(tenant_id,company_id,payroll_closing_id,payroll_event_id,event_type,amount,quantity,description,cost_center_id)
  select tenant_id,company_id,v_closing_id,id,event_type,amount,quantity,description,cost_center_id
  from public.payroll_events
  where tenant_id=p_tenant_id and company_id=p_company_id and employment_contract_id=p_employment_contract_id
    and competence_month=p_competence_month;
  ...
  values(p_tenant_id,p_company_id,v_user_id,'payroll.closed','payroll_closing',v_closing_id,
    jsonb_build_object('competence_month',p_competence_month,'employment_contract_id',p_employment_contract_id,'base_salary',v_salary,'event_credits',v_credit,'event_debits',v_debit));
  return v_closing_id;
end;
$$;
...
set search_path = ''
as $$
  select app_private.close_payroll_impl(p_tenant_id,p_company_id,p_employment_contract_id,p_competence_month,p_idempotency_key);
$$;
