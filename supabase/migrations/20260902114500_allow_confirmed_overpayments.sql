...
begin
  ...
  select exists(select 1 from public.financial_accounts fa where fa.tenant_id=p_tenant_id and fa.company_id=p_company_id and fa.id=p_account_id and fa.status='active') into v_account_active;
  if not v_account_active then raise exception 'active financial account not found in company'; end if;
  select coalesce(sum(fs.amount),0)::numeric(14,2) into v_settled_total from public.financial_settlements fs where fs.tenant_id=p_tenant_id and fs.company_id=p_company_id and fs.installment_id=p_installment_id;
  ...
  values(p_tenant_id,p_company_id,auth.uid(),'financial_settlement.recorded','financial_settlement',v_settlement_id,
    pg_catalog.jsonb_build_object('installment_id',p_installment_id,'account_id',p_account_id,'settled_on',p_settled_on,'amount',p_amount,'idempotency_key',v_key,'status_after',v_status,'overpayment',greatest(v_settled_total-v_installment_amount,0)));
...
begin
  ...
  select exists(select 1 from public.financial_accounts fa where fa.tenant_id=p_tenant_id and fa.company_id=p_company_id and fa.id=p_account_id and fa.status='active') into v_account_ok;
  ...
  values(p_tenant_id,p_company_id,p_account_id,p_paid_on,'outflow',p_amount,'card_statement_payment',v_payment_id,'Pagamento de fatura de cartão');
  ...
  values(p_tenant_id,p_company_id,auth.uid(),'card_statement_payment.recorded','card_statement_payment',v_payment_id,
    jsonb_build_object('statement_id',p_statement_id,'card_id',v_statement.card_id,'account_id',p_account_id,'paid_on',p_paid_on,'amount',p_amount,'idempotency_key',btrim(p_idempotency_key),'status_after',v_status,'overpayment',greatest(v_paid_total-v_statement.statement_amount,0)));
...
create or replace view public.credit_card_limits
...
with installment_totals as (
  ...
  select cs.tenant_id,cs.company_id,cs.card_id,cs.id as statement_id,
  ...
  group by cs.tenant_id,cs.company_id,cs.card_id,cs.id,cs.statement_amount
), payment_totals as (
  select tenant_id,company_id,card_id,coalesce(sum(applied_total),0)::numeric(14,2) as total
...
left join installment_totals it on it.tenant_id=cc.tenant_id and it.company_id=cc.company_id and it.card_id=cc.id
left join payment_totals pt on pt.tenant_id=cc.tenant_id and pt.company_id=cc.company_id and pt.card_id=cc.id;

comment on view public.credit_card_limits is 'Card limit commitment releases at most the formal statement amount per statement; confirmed cash overpayments remain in payment history without releasing future installments twice.';
