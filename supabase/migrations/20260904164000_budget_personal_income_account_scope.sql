...
with planned as (
  select tenant_id, company_id, cost_center_id, category_id, competence_month,
    sum(case when flow_type='income' then planned_amount else 0 end)::numeric(18,2) as planned_income,
    sum(case when flow_type='expense' then planned_amount else 0 end)::numeric(18,2) as planned_expense
  ...
  select bl.tenant_id, bl.company_id, bl.cost_center_id, bl.category_id, bl.competence_month,
    coalesce(sum(d.amount),0)::numeric(18,2) as actual_expense
  ...
  select fe.tenant_id, fe.company_id, fe.cost_center_id, fe.category_id,
    date_trunc('month',fs.settled_on)::date as competence_month,
    coalesce(sum(fs.amount),0)::numeric(18,2) as actual_income
  ...
  join public.financial_accounts fa on fa.id=fs.account_id and fa.tenant_id=fs.tenant_id and fa.company_id=fs.company_id
  join public.companies co on co.id=fe.company_id and co.tenant_id=fe.tenant_id
  where fe.entry_type='income'
    and fe.include_in_budget=true
    and (
      upper(trim(coalesce(co.trade_name,co.legal_name,''))) <> 'PESSOAL'
      or upper(trim(fa.name)) in ('NUBANK PESSOAL','DINHEIRO')
    )
...
select k.tenant_id,k.company_id,k.cost_center_id,k.category_id,k.competence_month,
  coalesce(p.planned_expense,0)::numeric(18,2) as planned_amount,
  coalesce(e.actual_expense,0)::numeric(18,2) as actual_expense,
  coalesce(i.actual_income,0)::numeric(18,2) as actual_income,
  (coalesce(p.planned_expense,0)-coalesce(e.actual_expense,0))::numeric(18,2) as expense_budget_balance,
  case when coalesce(p.planned_expense,0)>0 then round(coalesce(e.actual_expense,0)*100/coalesce(p.planned_expense,0),2) else 0 end as execution_percent,
  coalesce(p.planned_income,0)::numeric(18,2) as planned_income,
  coalesce(p.planned_expense,0)::numeric(18,2) as planned_expense,
  (coalesce(p.planned_income,0)-coalesce(i.actual_income,0))::numeric(18,2) as income_budget_balance,
  (coalesce(p.planned_income,0)-coalesce(p.planned_expense,0))::numeric(18,2) as planned_result,
  (coalesce(i.actual_income,0)-coalesce(e.actual_expense,0))::numeric(18,2) as actual_result,
  case when coalesce(p.planned_income,0)>0 then round(coalesce(i.actual_income,0)*100/coalesce(p.planned_income,0),2) else 0 end as income_execution_percent
from keys k
