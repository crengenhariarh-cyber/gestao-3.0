...
create policy financial_installments_delete_manager
on public.financial_installments
for delete
to authenticated
using (app_private.can_manage_company(tenant_id, company_id));

create policy financial_entries_delete_manager
...
begin
  ...
  delete from public.financial_installments
   where tenant_id=p_tenant_id and company_id=p_company_id and entry_id=p_entry_id;
...
create or replace function public.delete_unsettled_financial_entry(
  p_tenant_id uuid,
  p_company_id uuid,
  p_entry_id uuid
)
...
begin
  ...
  if exists (
    ...
    join public.financial_settlements s on s.tenant_id=i.tenant_id and s.company_id=i.company_id and s.installment_id=i.id
    where i.tenant_id=p_tenant_id and i.company_id=p_company_id and i.entry_id=p_entry_id
  ) then raise exception 'settled financial entry cannot be deleted'; end if;
...
revoke all on function public.delete_unsettled_financial_entry(uuid,uuid,uuid) from public, anon;
grant execute on function public.delete_unsettled_financial_entry(uuid,uuid,uuid) to authenticated;
