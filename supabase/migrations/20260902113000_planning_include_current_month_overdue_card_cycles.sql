...
select
  ...
  coalesce(nullif(substring(coalesce(fe.notes, '') from 'parcela=([0-9]+)/[0-9]+'), '')::integer, fib.installment_number) as installment_number,
  coalesce(nullif(substring(coalesce(fe.notes, '') from 'parcela=[0-9]+/([0-9]+)'), '')::integer, fib.installment_count) as installment_count,
  fib.due_date,
  fib.remaining_amount as amount,
...
select
  ...
  coalesce(
    cs.due_date,
    case
      when cc.due_day > cc.closing_day then (ci.statement_month + ((cc.due_day - 1)::text || ' days')::interval)::date
      else (ci.statement_month + interval '1 month' + ((cc.due_day - 1)::text || ' days')::interval)::date
    end
  ) as due_date,
  case when cs.id is not null then coalesce(sb.remaining_amount, 0)::numeric(14,2) else sum(ci.amount)::numeric(14,2) end as amount,
  case when cs.id is null then 'open'::text else coalesce(sb.payment_status, 'pending') end as payment_status
...
having
  (case when cs.id is not null then coalesce(sb.remaining_amount, 0) else sum(ci.amount) end) > 0
  and (
    cs.id is not null
    or (
      case
...
comment on view public.finance_planning_items is 'Pending installments plus active-card cycles. Open card cycles are retained from the current due month onward so yesterday/current-month overdue invoices remain visible without reviving older migrated ghost cycles.';
